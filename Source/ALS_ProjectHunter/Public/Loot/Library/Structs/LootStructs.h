...
#include "LootStructs.generated.h"
...
struct FLootEntry
{
	...
	{
		return ItemRowHandle.DataTable != nullptr || ItemClass.Get() != nullptr;
	}
...
struct FLootResult
{
	...
	bool IsValid() const { return Item.Get() != nullptr && Quantity > 0; }
};
...
struct FLootSpawnSettings
{
	...
	/** Use a box volume instead of circular scatter radius */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Spawn|Box")
	bool bUseSpawnBox = false;

	/**
	 * Half-extents of the spawn box (local space, centered on SpawnLocation).
	 * Set Z to 0 to keep spawns flat on the XY plane.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Spawn|Box", meta = (EditCondition = "bUseSpawnBox"))
	FVector SpawnBoxExtent = FVector(100.f, 100.f, 0.f);
	...
	FLootSpawnSettings()
		...
		, bUseSpawnBox(false)
		, SpawnBoxExtent(100.f, 100.f, 0.f)
	{}
};
