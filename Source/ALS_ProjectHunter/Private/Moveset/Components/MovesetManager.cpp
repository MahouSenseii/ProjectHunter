...
DEFINE_LOG_CATEGORY(LogMovesetManager);
	...
	const FMovesetInstance& Moveset, FMovesetInstance& OutPreviousMoveset)
{
	...
	{
		PH_LOG_WARNING(LogMovesetManager, "SocketMoveset failed: Moveset was invalid or EquipmentManager was null.");
		return false;
	}
	...
	if (!WeaponItem)
	{
		PH_LOG_WARNING(LogMovesetManager, "SocketMoveset failed: No weapon was equipped in Slot=%d.", static_cast<int32>(WeaponSlot));
		return false;
	}
	...
	{
		PH_LOG_WARNING(LogMovesetManager,
			"SocketMoveset failed: Moveset=%s was not compatible with Weapon=%s.",
