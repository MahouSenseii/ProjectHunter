...
void FItemStackingHelper::UpdateTotalWeight(UItemInstance& Item)
{
	Item.TotalWeight = Item.GetBaseWeight() * Item.Quantity;
}

bool FItemStackingHelper::IsStackable(const UItemInstance& Item)
{
	FItemBase* Base = Item.GetBaseData();
	return Base ? Base->bStackable : false;
}

bool FItemStackingHelper::CanStackWith(const UItemInstance& Item, const UItemInstance* Other)
{
...
bool FItemStackingHelper::IsConsumed(const UItemInstance& Item)
{
...
int32 FItemStackingHelper::AddToStack(UItemInstance& Item, int32 Amount)
{
...
int32 FItemStackingHelper::RemoveFromStack(UItemInstance& Item, int32 Amount)
{
	if (Amount <= 0)
	{
		return 0;
...
UItemInstance* FItemStackingHelper::SplitStack(UItemInstance& Item, int32 Amount)
{
...
int32 FItemStackingHelper::GetRemainingStackSpace(const UItemInstance& Item)
{
	if (!IsStackable(Item))
	{
		return 0;
