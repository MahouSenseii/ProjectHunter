...
void FItemUsageHelper::ApplyAffixesToCharacter(UItemInstance& Item, UAbilitySystemComponent* ASC)
{
...
void FItemUsageHelper::RemoveAffixesFromCharacter(UItemInstance& Item, UAbilitySystemComponent* ASC)
{
	if (!ASC)
	{
		return;
...
bool FItemUsageHelper::UseConsumable(UItemInstance& Item, AActor* Target)
{
	...
	{
		FItemStackingHelper::RemoveFromStack(Item, 1);
	}

	return true;
}

bool FItemUsageHelper::CanUseConsumable(const UItemInstance& Item)
{
...
float FItemUsageHelper::GetCooldownProgress(const UItemInstance& Item)
{
	FItemBase* Base = Item.GetBaseData();
...
bool FItemUsageHelper::ReduceUses(UItemInstance& Item, int32 Amount)
{
	Item.RemainingUses = FMath::Max(0, Item.RemainingUses - Amount);
	return Item.RemainingUses <= 0;
}

bool FItemUsageHelper::ApplyConsumableEffects(UItemInstance& Item, AActor* Target)
{
	FItemBase* Base = Item.GetBaseData();
	if (!Base)
	{
